...
def add_ldap_group(group, gid):
    ...
    try:
        response = ldap_conn.add(
            "gidNumber={},{}".format(gid, BASE_GROUP_DN),
            ["posixGroup", "top"],
            ldap_attr,
        )
        print(response[1])
...
def delete_ldap_group(gidNumber):
    try:
        ...
        if response[0] == True:
            return {
                "message": "Group {} deleted successfully".format(gidNumber),
                "status": "success",
            }
        elif response[0] == False:
...
def delete_ldap_user(user):
    try:
        ldap_conn = ldap_client("cn=admin,{}".format(LDAP_BASE), "admin")
        response = ldap_conn.delete("cn={},{}".format(user, BASE_USER_DN))
        print(response)
        if response[0] == True:
            return {
                "message": "User {} deleted successfully".format(user),
                "status": "success",
            }
        elif response[0] == False:
            return {"error": response[1]["description"], "status": "error"}
